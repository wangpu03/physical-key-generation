% input different transmission power with fixed power of noise
function [v12_ch,v21_ch,x_bd1,x_bd2] = func_ambc_ofdm_ray_exe_02(pt)
...
x_s = sqrt(pt)*(x_s/sqrt(mean(abs(x_s).^2)));
data_pwr = mean(abs(x_s).^2);
% add noise to channel 
noise_pwr = 0.1/10^(snr/10);
...
v12_s_noise_ch = v12_s_noise.*conj(x_s(n_L1:n_cpe)).*conj(x_s(n_L1:n_cpe))./(x_s(n_L1:n_cpe).*conj(x_s(n_L1:n_cpe))).^2;

v21_s_noise = zb2_s_noise.*zd2_s_noise;
v21_s_noise_ch = v21_s_noise.*conj(x_s(n_L1:n_cpe)).*conj(x_s(n_L1:n_cpe))./(x_s(n_L1:n_cpe).*conj(x_s(n_L1:n_cpe))).^2;

%% return the virtual channel information
